...
static bool unix_detect_thp(void) {
  ...
  if (fd >= 0) {
    ...
    if (nread >= 1) {
      // TH8 patch: bound the search to the bytes actually read (nread),
      // not the full buffer size.  buf is not zero-initialised, so
      // searching all 32 bytes read the uninitialised tail (flagged by
      // Valgrind in _mi_strnlen).  See externals/mimalloc/patches.
      thp_enabled = (_mi_strnstr(buf,(size_t)nread,"[never]") == NULL);
    }
  }
  #endif
  return thp_enabled;
